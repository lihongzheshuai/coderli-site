...
这两行数据表示，学号为 $7$ 的同学总分为 $279$，学号为 $5$ 的同学总分为 $279$。在不同的答卷中，双方总分都是 $279$，但学号为 $7$ 的同学语文成绩高一些，所以排在前面。
...
第 $2$ 到 $n + 1$ 行，每行有 $3$ 个空格隔开的数字，每个数字都在 $0$ 到 $100$ 之间，分别表示一个学生的语文、数学、英语成绩。学生学号按输入顺序编号为 $1 \sim n$（恰好是输入数据的行号减 $1$）。
